...
def _classify_evidence(txn: Transaction, raw_evidence: dict) -> tuple[list[str], list[str]]:
    ...
    if not supporting:
        supporting.append("no additional supporting evidence found beyond the original signals")
    if not contradicting:
        contradicting.append("no contradicting evidence found in customer/device/transaction history")
