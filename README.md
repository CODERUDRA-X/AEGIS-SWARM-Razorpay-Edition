...
- **10 of 12 misses**: the baseline ML `risk_score` was already below 0.5 — the
  baseline model itself would have missed these too. This is an **ML
  feature-signal gap** (or a label-noise-flipped row), not something the
  agent layer introduced, since agents only reason over transactions the
  baseline already scored.
...
Three hand-constructed, clearly-labeled scenarios for live demonstration:

- **Case A — Clear Fraud**: new device/account, IP/billing mismatch, velocity
  spike → expected BLOCK.
- **Case B — Ambiguous**: elevated signals but long-standing, consistent
  account history → expected STEP_UP or REVIEW (evidence conflicts with the
  initial hypothesis).
- **Case C — Legitimate**: normal amount/timing, established account, known
  device → expected ALLOW.

These are demo fixtures for illustrating the decision paths, explicitly separate
from and never mixed into the held-out evaluation-set statistics above.
